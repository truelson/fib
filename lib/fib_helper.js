...
FibHelper.createFibHelper = function( dom, resourceDir, hasAlerts ) {
  ...
  fibHelper.scalePathFill = function( obj, dom ) {

    if( !obj || !obj.pathAttributes || !obj.pathAttributes.fillHandle1 ) return

    var oldSelection = fw.selection
    fw.selection = [ obj ]

    var start = { x: obj.pathAttributes.fillHandle1.x, y: obj.pathAttributes.fillHandle1.y }
    var end1 = { x: obj.pathAttributes.fillHandle2.x, y: obj.pathAttributes.fillHandle2.y }
    var end2 = { x: obj.pathAttributes.fillHandle3.x, y: obj.pathAttributes.fillHandle3.y }

    dom.moveFillVectorHandleBy({ x: start.x,
                                 y: start.y  },
                               "start", false, false )

    dom.moveFillVectorHandleBy({ x: end1.x - start.x,
                                 y: end1.y - start.y  },
                               "end1", false, true )

    dom.moveFillVectorHandleBy({ x: end2.x - start.x,
                                 y: end2.y - start.y  },
                               "end2", false, true )
  ...
  fibHelper.scaleAllPathFill = function( obj, dom ) {

    if( !obj ) return

    fibHelper.scalePathFill( obj, dom )

    _.map( obj.elements, function( childObj ) {
      fibHelper.scaleAllPathFill( childObj, dom )
    })
  }
  ...
  fibHelper.exportPNG = function ( obj, name ) {
    ...
    var newObj = newDOM.layers[0].elems[0]
    fibHelper.scaleAllPathFill( newObj, newDOM );
  ...
  fibHelper.getActualSize = function getActualSize( obj ) {
    ...
    if ( fibHelper.objectType( obj ) === 'Text' ) {
      size.left = obj.rawLeft
      size.top = obj.rawTop
      size.width = obj.rawWidth
      size.height = obj.rawHeight
    }
