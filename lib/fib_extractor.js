...
FibExtractor.extractImage = function( fireworksObject, outObject, parent) {
  ...
  if ( outObject.opacity ) delete outObject.opacity
...
FibExtractor.extractorType.Layer = function( fireworksObject, parent ) {
...
FibExtractor.extractorType.Group = function( fireworksObject, parent ) {

  var outObject      = FibExtractor.extractBasic( fireworksObject, parent )
  ,   groupExtractor = FibExtractor.extractorTypeSetup()
...
FibExtractor.extractorType.Path = function( fireworksObject, parent ) {

  var outObject     = FibExtractor.extractBasic( fireworksObject, parent )
  ,   isImage       = FibExtractor.shouldExtractVectorAsImage(
    fireworksObject )
  ,   pathExtractor = FibExtractor.extractorTypeSetup()
  ...
  pathExtractor.canvas = function( fireworksObject, outObject, parent ) {
    ...
    _(fireworksObject.contours).each( function ( contour ) {
      ...
      if ( fireworksObject.pathAttributes.brush ) {
        ...
        outContour.linewidth = fireworksObject.pathAttributes.brush.diameter
      }
...
FibExtractor.extractorType.Image = function( fireworksObject, parent ) {

  var outObject      = FibExtractor.extractBasic( fireworksObject, parent )
  ,   imageExtractor = FibExtractor.extractorTypeSetup()

  if ( typeof outObject.type === 'undefined' ) outObject.type = 'image'

  if ( outObject.type === 'canvas' ) {
    outObject.type = 'image'
    alert( fireworksObject.name + ' cannot be a canvas as it is an image' )
  }
...
FibExtractor.extractorType.Instance = FibExtractor.extractorType.Image

FibExtractor.extractorType.Text = function( fireworksObject, parent ) {

  var outObject     = FibExtractor.extractBasic( fireworksObject, parent )
  ,   textExtractor = FibExtractor.extractorTypeSetup()
  ...
  textExtractor.textfield = function( fireworksObject, outObject, parent) {
    outObject = FibExtractor.extractTextfield( fireworksObject, outObject )
...
FibExtractor.extract = function( fireworksObject, parent ) {
  ...
  if ( FibExtractor.extractorType.hasOwnProperty( fireworksType )) {
    return FibExtractor
      .extractorType[ fireworksType ]( fireworksObject, parent )
  }
